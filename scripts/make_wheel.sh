...
#
# Produces a reproducible wheel, sdist, and SHA256SUMS in vibe-view/dist/.
# The validated wheel is then staged under docs/_static/downloads/ for the
# website download link, mirroring qvf-writer/scripts/make_archive.sh.  An
...
# docs/tutorial/vibe_view_getting_started.md.
#
# Usage:  vibe-view/scripts/make_wheel.sh
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
VIEWER_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
REPO_DIR="$(cd "${VIEWER_DIR}/.." && pwd)"
DOWNLOADS="${REPO_DIR}/docs/_static/downloads"
...
grep -rl 'vibeview-.*-py3-none-any\.whl' "${REPO_DIR}/docs" --include='*.md' 2>/dev/null \
  | sed 's|^|  |' || true
