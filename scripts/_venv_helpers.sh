...
vibe_view_refresh_desktop() {
    ...
    "$venv/bin/python" "$installer" "${args[@]}"
}

VIBE_VIEW_BIN_LINK_MARKER="# Generated by vibe-view install.sh --link-bin"
VIBE_VIEW_BIN_LINK_RECORD=".vibe-view-bin-links"

vibe_view_link_bin() {
    local venv="$1"
    local dir="$2"
    local link="$dir/vibe-view"
    local record="$VIBE_VIEW_PROJECT_DIR/$VIBE_VIEW_BIN_LINK_RECORD"
...
EOF
    ...
    { grep -Fxv "$dir" "$record" 2>/dev/null || true; echo "$dir"; } \
